  ...
  <aside class="flex flex-col h-full bg-white border-r border-gray-200 w-[280px] shrink-0 z-20 shadow-sm relative">
    <div class="p-4 border-b border-gray-200 bg-gray-50 shrink-0">
      <input v-model="searchQuery" type="text" placeholder="搜索图标或警示语..."
        class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm outline-none focus:border-[#1677ff] focus:ring-1 focus:ring-[#1677ff] transition-all">
    </div>
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar pb-10">
      ...
      <div v-for="(category, catIndex) in filteredLibrary" :key="catIndex" class="mb-2">
        <div
          class="px-4 py-3 bg-gray-100 border-b border-gray-200 border-l-4 border-l-[#1677ff] font-extrabold text-sm text-gray-800 sticky top-0 z-10 shadow-sm">
          {{ category.parentTitle }}
        </div>

        <div v-for="(group, grpIndex) in category.groups" :key="grpIndex">
          <div
            class="flex justify-between items-center px-4 py-2.5 bg-gray-50 border-b border-gray-100 text-xs font-bold text-gray-500 cursor-pointer hover:bg-gray-200 transition-colors"
            ...
            <svg :class="{ 'rotate-[-90deg]': !expandedGroups.has(group.groupTitle) }"
              class="w-3.5 h-3.5 transition-transform" viewBox="0 0 24 24" fill="none" stroke="currentColor"
              stroke-width="2">
          ...
          <ul v-show="expandedGroups.has(group.groupTitle)">
            <li v-for="(item, idx) in group.items" :key="idx"
              ...
              <div class="flex justify-between items-center px-4 py-3 cursor-pointer" @click="toggleItem(item.name)">
                ...
                <svg :class="{ 'rotate-180 text-[#1677ff]': expandedItems.has(item.name) }"
                  class="w-4 h-4 text-gray-400 transition-transform" viewBox="0 0 24 24" fill="none"
              ...
              <div v-if="expandedItems.has(item.name)"
                class="px-4 py-3 bg-gray-50 border-t border-dashed border-gray-200">
                <img v-if="item.type === 'image' || item.type === 'image_text'" :src="item.img"
                  class="max-w-[140px] max-h-[80px] object-contain mb-2 cursor-pointer hover:ring-2 hover:ring-[#1677ff] rounded shadow-sm bg-white p-1"
                ...
                <div v-if="item.type === 'text' || item.type === 'image_text'"
                  class="whitespace-pre-wrap break-words p-2 bg-white border border-gray-200 rounded text-xs text-gray-800 font-medium cursor-pointer hover:ring-2 hover:ring-[#1677ff] shadow-sm"
